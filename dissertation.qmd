...
flowchart TB
    ...
    imps --> iterate{"`Non-<br>convergence?`"}
    imps --> misfit{"`Misfit?`"}    
    subgraph 1["Inspect convergence \& imputed values"]
    iterate{"`Non-<br>convergence?`"}
    misfit{"`Misfit?`"}
    end
    iterate -- "yes" --> imp
    iterate -- "no" --> analyses[\"`Run analyses`"/]
    misfit -- "yes" --> mod
...
```
    ...
    <!-- imps --> check[\"`Inspect imputed values`"/] -->
