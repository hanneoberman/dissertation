...
```{dot}
digraph MultipleImputation {
    rankdir=LR;

    // Default node style: empty circles
    node [shape=circle, width=1.1, fixedsize=true, label=""];

    // Column 1: single incomplete-data circle
    I;

    // Column 2: three imputed-data circles
    IM1; IM2; IM3;

    // Column 3: three analysis-results circles
    AN1; AN2; AN3;

    // Column 4: single pooled-result circle
    P;

    // Vertical alignment for each column
    { rank=same; IM1; IM2; IM3 }
    { rank=same; AN1; AN2; AN3 }

    // Arrows matching the original figure
    I  -> IM1;
    I  -> IM2;
    I  -> IM3;
    ...
    // Column labels (placed under representative node)
    I   [xlabel="Incomplete data"];
    IM2 [xlabel="Imputed data"];
    AN2 [xlabel="Analysis results"];
    P   [xlabel="Pooled result"];
}
```
...
ggmice(a, aes(X, Y)) + 
  ...
  theme_classic()
# dat_xy[6, 2] <- 7.8
# mod <- lm(Y ~ X, data = dat_xy) 
# b <- mod |> coef()
# ci <- confint(mod)
# est <- paste0(round(b[2], 2), " [", round(ci[2,1], 2), ", ", round(ci[2,2], 2), "]")
# ggmice(dat_xy, aes(X, Y)) + 
#   geom_point() + 
#   geom_smooth(method = "lm", se = FALSE) + 
#   theme_classic() + 
#   labs(title = paste("Regression coefficient =", est))
# dat_xy[6, 1] <- 8.2
# mod <- lm(Y ~ X, data = dat_xy) 
# b <- mod |> coef()
# ci <- confint(mod)
