...
Missing data may be widespread, but so are missing data treatment methods. In this dissertation, I set out to investigate what is needed to develop, apply, and evaluate the popular missing data method 'MICE'.

 Imputation is a convenient and popular missing data method. 

Although imputation is widely supported with software, and many implementations of the method have convenient defaults, generating *good* imputations is very hard still. 
...
- this generates several complete*d* datasets.
...
- stat eval not possible IRL? bias/cov impossible to determine without assumed truth --> only wrt estimand with PPC (e.g. bootstrapping is also stat/comp)
...
- --> can we use existing tools or do we need more? and how? is the status quo sufficient? no, and I contribute to the new status quo ("verleggen")
- **wat is er nodig om ontwikkeling en evaulatie en toepassing van imp methods te waarborgen?** stat eval is required, but not sufficient. how Rubin developed the methodology is not enough & is this present in imputors toolbox? no, that's what I'm doing
- verenigen van hoe maak en evalueer je imputaties
:::
...
- Model building: how do we know which imputation model predictors to select?
  - association of incomplete variable with other variables in the data ($\rightarrow$ `ggmice` chapter) ["include an auxiliary variable if its correlation with the main variable is ≥0.5 (in absolute value) [25]" @nguyen2021]
...
- Model development: how can we compare imputation methods? $\rightarrow$ evaluation chapter
...
* only diffs, not included what all packages can --> maybe as appendix?

TODO: paragraph to explain all aspects of the table.
