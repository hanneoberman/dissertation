...
The ideal route of studying the formal statistical properties of the method via analytical derivations is often impossible for imputation algorithms. Instead, the de facto standard way is to estimate its empirical performance via simulation studies, but these are not standardized across studies and therefore hard to compare. 
Moreover, simulation study results might not translate to applied use. 
And finally, there is no systematic format for evaluating the practical performance of imputation algorithms in real-world applications, such as diagnostics for algorithmic convergence. 
Evaluating whether imputations should be trusted remains challenging. 
...
I am more inclined to call myself a methodologist rather than statistician. 
...
Standard errors then get translated into decision tools for inference such as $p$-values and confidence intervals (CIs).
These help researchers distinguish between real-world associations (or signal) and noise. 
Statistics are a means of estimating effects within a sample and inferring back to the population with some expression of uncertainty.
For example, when sociologists wanted to study the trust in the government during the COVID-19 pandemic, they did not survey all Dutch citizens, but instead used a representative sample. 
However, when there is just one incomplete observation, statistical inference cannot be performed. 
With missing values in a dataset, even simple statistics such as the mean of variables are mathematically undefined.
It is not possible to use an incomplete sample to infer about a larger population without some kind of missing data method. 
...
classDef hidden display: none;
  ...
  C2 --inference with<br>missing data<br>method--> A2
```
...
Even without a missing data method, there is an implicit missing data method. 
...
In that study about trusting the government during the Covid 19 pandemic, it was not random who had missing data on the question about trust in the government. 
