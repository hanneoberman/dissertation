...
For data analysts, missing data are a nuisance. Missingness stands in the way of unbiased and confidence-valid statistical inferences when the incomplete data at hand is in any way different from the data one would like to analyze. For example, when drop-out in a clinical trial is related to the severity of side-effects from the treatment. Whenever missing data cannot be prevented, the only way forward is to treat the missing data problem. Missing values force data analysts to treat the missingness in some way. Even a common default such as list-wise deletion is a type of missing data method, which can severely impact the accuracy and validity of the subsequent analyses. Therefore, missing data methods should be applied carefully. Missing data may be widespread, but so are missing data treatment methods. In this dissertation, I set out to investigate what is needed to develop, apply, and evaluate the popular missing data method 'MICE'.

Technological advances have made treating missing data easier than ever. A single line of code can 'impute' every missing data point in an incomplete dataset. To 'impute' means to fill in, so 'imputation' is the practice of replacing missing values with synthetic substitutes. Imputation splits the missing data problem from the analysis of scientific interest. Imputation allows data analysts to first fill in the missingness, to then be able to analyze the completed data as if they were completely observed. Imputation is a convenient and popular missing data method. 
