...
- Missing data are widespread, and so is imputation.
...
<!-- TODO check req info: imputation = filling in. then phil about evaluation, then deeper into imputation -->
...
- Since the term is not defines for statistics, there is no definition for imputation methodology.
...
- Imputation thoery does not require the imputed values to be plausible. 
...
- computational evaluation at different levels: 
  - building imputation models (`ggmice`), 
  - assessing imputation models (`ggmice` and convergence), 
  - developing new imputation models/methods (evaluation paper)
...
- GitHub, CRAN, Zenodo, ...
- providing tools to see obs and imputation side-by-side
